...
export const sendVerificationEmail = async (email, verificationToken) => {
  ...
  try {
    ...
    console.error(`Error sending verifcation email`, error);
...
export const sendWelcomeEmail = async (email, name) => {
  ...
  try {
    const response = await mailtrapClient.send({
      from: sender,
      to: recipient,
      template_uuid: "6fdede84-7982-4225-88aa-127e13d3e938",
      template_variables: {
        name: name,
      },
    });
...
export const sendPasswordResetEmail = async (email, resetURL) => {
  ...
  try {
    ...
    console.log("forgot email sent successfully", response);
  } catch (error) {
